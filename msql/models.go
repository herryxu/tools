...
import (
	...
	"errors"
	"fmt"
	"gitlab.zmwk.cn/open/open-golang-tools/tool"
	"strings"
)
...
func (m *db) getFields() string {
	fields := tool.Array2String(m.field, ",")
	if fields == "" {
		return "*"
	}
	return tool.GetStringUnique(fields, ",")
}
...
func (m *db) getLimit() string {
	...
	return "limit " + tool.Int2String(m.offset) +
		"," + tool.Int2String(m.limit)
}
...
func (m *db) Insert(data Datas) (int64, error) {
	if len(data) < 1 {
